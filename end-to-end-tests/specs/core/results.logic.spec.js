...
describe("single study query", function() {
  ...
  describe("enrichments", function() {
    ...
    it("should show mutations plot", function() {
      goToUrlAndSetLocalStorage(
        `${CBIOPORTAL_URL}/index.do?cancer_study_id=ov_tcga_pub&Z_SCORE_THRESHOLD=2.0&RPPA_SCORE_THRESHOLD=2.0&data_priority=0&case_set_id=ov_tcga_pub_cna_seq&gene_list=BRCA1+BRCA2&geneset_list=+&tab_index=tab_visualize&Action=Submit&genetic_profile_ids_PROFILE_MUTATION_EXTENDED=ov_tcga_pub_mutations&genetic_profile_ids_PROFILE_COPY_NUMBER_ALTERATION=ov_tcga_pub_gistic`
      );
...
describe("case set selection in modify query form", function() {
  ...
  it('contains correct selected case set through a certain use flow involving the "select all filtered studies" checkbox', () => {
    ...
    browser.waitForExist(selectedCaseSet_sel, 10000);
    browser.waitUntil(() => {
      return browser.getText(selectedCaseSet_sel) === "All (2263)";
    }, 5000);
