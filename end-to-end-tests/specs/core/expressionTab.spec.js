...
describe("results page expression tab", () => {
  before(() => {
    goToUrlAndSetLocalStorage(
      CBIOPORTAL_URL +
        "/results/expression?cancer_study_id=all&Z_SCORE_THRESHOLD=2&RPPA_SCORE_THRESHOLD=2&data_priority=0&case_set_id=all&gene_list=EGFR&geneset_list=+&tab_index=tab_visualize&Action=Submit&cancer_study_list=chol_tcga_pan_can_atlas_2018%2Cchol_tcga%2Cblca_dfarber_mskcc_2014"
    );
  });
  ...
  it("study selection modal filters work properly", function() {
    ...
    assert.equal(
      browser.isVisible('[data-test="ExpressionStudyModalButton"]'),
      true,
      "modal is visible"
    );

    var studyCheckboxes = browser.elements(
      '[data-test="ExpressionStudyModal"] input[type=checkbox]'
    ).value;
    ...
    assert.equal(
      ...
      2,
      "all studies with data selected by default"
    );
    ...
    assert.equal(
      getSelectedStudies(studyCheckboxes).length,
      2,
      "all studies with data selected"
    );
  });
});
