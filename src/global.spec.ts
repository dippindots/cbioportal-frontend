...
request.get = function() {
  console.log(
    `Warning: calling stubbed method request.get, which returns a promise that never resolves.`
  );
  return new Promise(() => {});
} as any;
...
request.post = function() {
  console.log(
    `Warning: calling stubbed method request.post, which returns a promise that never resolves.`
  );
  return new Promise(() => {});
} as any;
...
function stubClient(): any[] {
  ...
  for (var funcKey in arrObj) {
    ...
    (CBioPortalAPI.prototype as any)[methodName] = function() {
      console.log(
        `Warning: calling stubbed method cbioportalClient.${methodName}, which returns a promise that never resolves.`
      );
    };
  }
