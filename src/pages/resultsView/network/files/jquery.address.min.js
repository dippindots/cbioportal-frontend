  ...
  c.address = (function() {
    var s = function(a) {
        a = c.extend(
          ...
          function() {
            for (
              var b = {}, f = c.address.parameterNames(), m = 0, p = f.length;
              m < p;
              m++
            )
      ...
      T = function() {
        return (
          "/" +
          n.pathname.replace(new RegExp(d.state), "") +
          n.search +
          (H() ? "#" + H() : "")
        ).replace(S, "/");
      },
      ...
      ea = function() {
        if (d.tracker !== "null" && d.tracker !== D) {
          ...
          if (c.isFunction(a)) a(b);
            ...
            i.pageTracker !== h &&
            c.isFunction(i.pageTracker._trackPageview)
          )
            i.pageTracker._trackPageview(b);
          else
            i._gaq !== h &&
              c.isFunction(i._gaq.push) &&
              i._gaq.push(["_trackPageview", decodeURI(b)]);
        }
      },
      N = function() {
        var a =
          ...
          (l.domain != n.hostname ? '";document.domain="' + l.domain : "") +
      ...
      Z = function() {
        if (E && Y != -1) {
          ...
          for (a = 0; a < f.length; a++) {
            b = f[a].split("=");
            if (/^(autoUpdate|history|strict|wrap)$/.test(b[0]))
              d[b[0]] = isNaN(b[1])
                ? /^(true|yes)$/i.test(b[1])
                : parseInt(b[1], 10) !== 0;
        ...
        if (!$) {
          ...
          if (d.wrap) {
            ...
            c("body > *")
              .wrapAll(
                '<div style="padding:' +
                  (t(a, "marginTop") + t(a, "paddingTop")) +
                  "px " +
                  (t(a, "marginRight") + t(a, "paddingRight")) +
                  "px " +
                  (t(a, "marginBottom") + t(a, "paddingBottom")) +
                  "px " +
                  (t(a, "marginLeft") + t(a, "paddingLeft")) +
                  'px;" />'
              )
              .parent()
              .wrap(
                '<div id="' +
                  ...
                  (K && !window.statusbar.visible ? "resize:both;" : "") +
                  '" />'
              );
            c("html, body").css({
              height: "100%",
            ...
            K &&
              c('<style type="text/css" />')
                .appendTo("head")
                .text(
                  "#" + y + "::-webkit-resizer { background-color: #fff; }"
                );
          }
          if (v && !J) {
          ...
          if (!G())
            if ((v && x > 7) || (!v && J))
              if (i.addEventListener) i.addEventListener(F, C, o);
      ...
      r = true,
      o = false,
      d = { autoUpdate: r, history: r, strict: r, wrap: o },
      z = (function() {
        var a = {},
    ...
    if (v) {
      ...
      if (l.documentMode && l.documentMode != x)
        x = l.documentMode != 8 ? 7 : 8;
      ...
      state: function(a) {
        if (a !== h) {
          ...
          if (d.state !== h)
            if (A.pushState)
              b.substr(0, 3) == "/#/" &&
                n.replace(d.state.replace(/^\/$/, "") + b.substr(2));
            else
              b != "/" &&
                b.replace(/^\/#/, "") != H() &&
                u(function() {
      ...
      value: function(a) {
        if (a !== h) {
          ...
          if (d.autoUpdate || L) {
            if (B(r)) return this;
            if (G())
              A[d.history ? "pushState" : "replaceState"](
                {},
                "",
                d.state.replace(/\/$/, "") + (e === "" ? "/" : e)
              );
      ...
      this.on("click", function(g) {
        ...
        if (c(k).is("a")) {
          ...
          g = s
            ...
            ? c(k)
                .attr("href")
                .replace(new RegExp("^(.*" + c.address.state() + "|\\.)"), "")
            : c(k)
                .attr("href")
                .replace(/^(#\!?|\.)/, "");
          c.address.value(g);
