...
function setUpTabs() {
  ...
  if (matches && matches.length > 1) {
    ...
    var tabs = $("#tabs > ul li a")
      ...
      .toArray();
    matchedIndex = _.findIndex(tabs, function(str) {
      return term === str;
    });
  }
  var activeIndex = matchedIndex === -1 ? null : matchedIndex;

  var summaryIsDefault =
    window.isVirtualStudy &&
    window.oql_parser.parse(serverVars.theQuery).length === 1;
...
function setUpTestimonials() {
  ...
  if ($("#all_testimonials").length > 0) {
    ...
    $("#testimonials blockquote").append(
      '<br><a class="wpasA" href="what_people_are_saying.jsp">View All</a>'
    );
    //rotateTestimonials();
...
function setUpPopEye() {
  ...
  setInterval(function() {
    if (
      !$("#custom_case_set_dialog").dialog("isOpen") &&
      !hovering &&
      maxNextClicks > 0
    ) {
      maxNextClicks--;
...
function bitlyURL(fullURL) {
  ...
  $.getJSON(qurl, function(data) {
    if (data.results == null) {
      $("#bitly").html(
        "An unknown error occurred. Unable to shorten your URL."
      );
    } else {
      $("#bitly").html(
        "<br><strong><a href='" +
          data.results[fullURL].shortUrl +
          "'>" +
          data.results[fullURL].shortUrl +
          "</a></strong><br>"
      );
...
function changeURLToSessionServiceURL(fullURL, pageTitle, sessionJSON) {
  getSessionServiceBookmark(fullURL, sessionJSON, function(bookmark) {
    window.history.replaceState(
      { html: fullURL, pageTitle: pageTitle },
      "",
      bookmark
    );
  });
...
function displayBookmark(bookmarkURL) {
  if (bookmarkURL === null) {
    $("#session-id").html(
      "An unknown error occurred. Unable to store your session."
    );
  } else {
    $("#session-id").html(
