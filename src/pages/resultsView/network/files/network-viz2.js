    ...
    style: cytoscape
      ...
      .css({
        ...
        "background-color": function(ele) {
          ...
          if (
            ele._private.data["type"] === "Drug" &&
            ele._private.data["FDA_APPROVAL"] === "true"
          ) {
            return "#E6A90F";
          ...
          if (value >= high) {
            return (
              "rgb" + "(" + highCRed + "," + highCGreen + "," + highCBlue + ")"
            );
          } else if (value > low) {
            return (
              "rgb" +
              "(" +
              getValueByRatio(value, low, high, lowCRed, highCRed) +
              "," +
            ...
            return (
              "rgb" + "(" + lowCRed + "," + lowCGreen + "," + lowCBlue + ")"
            );
          }
        },
        content: function(ele) {
          if (
            ele._private.data["label"] === undefined ||
      ...
      function updateDetailsTab(event) {
        if (tapped || edge) {
          ...
          for (var i = 0; i < nodes.length; i++) {
            ...
            nodes[i].css(
              "opacity",
              nodes[i]._private.style["main-opacity"].value
            );
      ...
      cy.on("tap", "node", function(evt) {
        ...
        if (!this.isParent()) {
          var nodes = cy.filter(function(i, element) {
            return (
              element.isNode() &&
              element._private.style["show-details-selected"] === true
            );
          });
          for (var i = 0; i < nodes.length; i++) {
            nodes[i]._private.style["show-details"] = false;
