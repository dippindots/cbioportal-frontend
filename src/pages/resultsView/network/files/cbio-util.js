...
cbio.util = (function() {
  ...
  var stringListDifference = function(from, by) {
    return Object.keys(
      objectKeyDifference(stringListToObject(from), stringListToObject(by))
    );
  };
  ...
  var alterAxesAttrForPDFConverter = function(
    xAxisGrp,
    shiftValueOnX,
    yAxisGrp,
    shiftValueOnY,
    ...
    var xLabels = xAxisGrp.selectAll(".tick").selectAll("text");

    var yLabels = yAxisGrp.selectAll(".tick").selectAll("text");
  ...
  function getLinkToPatientView(cancerStudyId, patientId) {
    return "/patient?studyId=" + cancerStudyId + "&caseId=" + patientId;
  }

  function getLinkToSampleView(cancerStudyId, sampleId) {
    return "/patient?studyId=" + cancerStudyId + "&sampleId=" + sampleId;
  }
  ...
  function getHotSpotDesc(isHotspot, is3dHotspot) {
    ...
    strBuilder.push(
      "in a population-scale cohort of tumor samples of " +
    ...
    if (isHotspot) {
      strBuilder.push(
        '<a href="https://www.cancerhotspots.org/" target="_blank">https://www.cancerhotspots.org/</a>'
      );
    ...
    if (is3dHotspot) {
      strBuilder.push(
        '<a href="https://www.3dhotspots.org/" target="_blank">https://www.3dhotspots.org/</a>.'
      );
    }
    // end links
  ...
  function getDatahubStudiesList() {
    var DATAHUB_GIT_URL = "proxy/download.cbioportal.org/study_list.json";
    var def = new $.Deferred();
  ...
  function getDecimalExponents(data) {
    ...
    if (!_.isArray(data) || data.length < 1) {
      //if data is not an array or is empty, return data
      return data;
    }
   ...
   */
  function showCombinedStudyNameAndDescription(
    nameSelector,
    descriptionSelector,
    studies,
    name,
    ...
    if (hasStudies) {
      ...
      description += twoMoreStudies
        ? "This combined study contains samples from " +
          studies.length +
          " studies."
        : getCollapseStudyName(studies);
    }
    $(descriptionSelector).append(description);
    if (twoMoreStudies) {
      var collapseStudyName = studies
        .map(function(study) {
  ...
  function getOriginStudiesDescriptionHtml(studies) {
    var originStudiesDescription = [
      '<div class="origin-studies"><div class="header">This virtual study was derived from:</div><div class="panel-group">'
    ];
    _.each(studies, function(study) {
      originStudiesDescription.push(
        '<div class="panel panel-default">' +
          ...
          'data-target="#' +
          study.id +
          '-collapse-content" class="collapsed">' +
          study.name +
          '</span><a target="_blank" href="' +
          window.cbioURL +
          "study?id=" +
          study.id +
          '"><i class="fa fa-external-link" aria-hidden="true"></i></a></h4></div>' +
          '<div id="' +
          study.id +
          '-collapse-content" ' +
  ...
  function addFoldableDescription(descriptionSelector, content) {
    $(descriptionSelector).append(
      '<span class="truncated"><br />' + content + "</span>"
    );
    var truncatedElm = $(descriptionSelector + " .truncated")
      .hide() // Hide the text initially
      .before(
        '<i class="fa fa-plus-circle combined-study-title-toggle-icon" aria-hidden="true" style="margin-left: 5px;"></i>'
      ); /// Create toggle button
    $(descriptionSelector)
      .find(".combined-study-title-toggle-icon")
      .on("click", function() {
        // Attach behavior
