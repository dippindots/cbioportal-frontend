window.cbioportal_client = (function() {
  var raw_service = (function() {
    ...
    var functionNameToEndpointProperties = {
      ...
      SampleClinicalAttributes: {
        endpoint: "api-legacy/clinicalattributes/samples"
      },
      PatientClinicalAttributes: {
        endpoint: "api-legacy/clinicalattributes/patients"
      },
      ...
      SampleListsMeta: {
        endpoint: "api-legacy/samplelists",
        args: { metadata: true }
      },
    ...
    for (var i = 0; i < key_list_list.length; i++) {
      ...
      for (var j = 0; j < key_list.length; j++) {
        for (var k = 0; k < ret.length; k++) {
          intermediate_ret.push(
            ret[k].concat([
              list_of_lists ? [].concat(key_list[j]) : key_list[j]
            ])
          );
        }
      }
  ...
  function HierIndex(key) {
    ...
    var flatten = function(list_of_lists) {
      ...
      for (var k = 0; k < n_chunks; k++) {
        flattened.push(
          [].concat.apply(
            [],
            list_of_lists.slice(k * chunk_size, (k + 1) * chunk_size)
          )
        );
      }
      ...
      while (intermediate.length > 0) {
        ...
        for (i = 0; i < intermediate.length; i++) {
          ...
          if (Object.prototype.toString.call(obj) === "[object Array]") {
            ...
            var keys =
              (key_list_index < key_list_list.length &&
                key_list_list[key_list_index]) ||
              Object.keys(obj);
    ...
    this.missingKeys = function(key_list_list) {
      ...
      var missing_keys = key_list_list.map(function() {
        return {};
      });
      var j, k;
      ...
      return missing_keys.map(function(o) {
        ...
        try {
          if (args.hasOwnProperty(arg_name)) {
            ...
            if (missing_keys.length > 0) {
              ...
              raw_service[service_fn_name](webservice_args)
                .then(function(data) {
                  index.addData(data);
  ...
  var makeTwoIndexService = function(
    arg_name1,
    indexKeyFn1,
    index1_always_add,
    arg_name2,
  ...
  var makeHierIndexService = function(
    arg_names,
    indexing_attrs,
    service_fn_name
  ) {
    return (function() {
      ...
      return function(args) {
        ...
        try {
          ...
          while (
            typeof arg_list_list[arg_list_list.length - 1] === "undefined"
          ) {
            arg_list_list.pop();
          }
  ...
  var enforceRequiredArguments = function(fnPtr, list_of_arg_combinations) {
    return function(args) {
      ...
      if (!matches_one) {
        var def = new $.Deferred();
        var msg =
          "Given arguments not acceptable; need a combination in the following list: ";
        msg += list_of_arg_combinations
          .map(function(arg_combo) {
            return arg_combo.join(",");
          })
          .join(";");
        def.reject({ msg: msg });
        return def.promise();
  ...
  var cached_service = {
    getCancerTypes: enforceRequiredArguments(
      makeOneIndexService(
        "cancer_type_ids",
        function(d) {
          return d.id;
    ...
    getGenes: enforceRequiredArguments(
      makeOneIndexService(
        "hugo_gene_symbols",
        function(d) {
    ...
    getStudies: enforceRequiredArguments(
      makeOneIndexService(
        "study_ids",
        function(d) {
          return d.id;
    ...
    getGenePanelsByPanelId: enforceRequiredArguments(
      makeOneIndexService(
        "panel_id",
        function(d) {
          return d.stableId;
    ...
    getGeneticProfiles: enforceRequiredArguments(
      makeTwoIndexService(
        "study_id",
        function(d) {
          return d.study_id;
      ...
      [["study_id"], ["genetic_profile_ids"]]
    ),
    getSampleLists: enforceRequiredArguments(
      makeTwoIndexService(
        "study_id",
        function(d) {
          return d.study_id;
      ...
      [["study_id"], ["sample_list_ids"]]
    ),
    getSampleClinicalData: enforceRequiredArguments(
      makeHierIndexService(
        ["study_id", "attribute_ids", "sample_ids"],
        ["study_id", "attr_id", "sample_id"],
        "getSampleClinicalData"
      ),
      [
        ["study_id", "attribute_ids"],
        ["study_id", "attribute_ids", "sample_ids"]
      ]
    ),
    getPatientClinicalData: enforceRequiredArguments(
      makeHierIndexService(
        ["study_id", "attribute_ids", "patient_ids"],
        ["study_id", "attr_id", "patient_id"],
    ...
    getPatients: enforceRequiredArguments(
      makeHierIndexService(
        ["study_id", "patient_ids"],
        ["study_id", "id"],
        "getPatients"
      ),
      [["study_id"], ["study_id", "patient_ids"]]
    ),
    getSamples: enforceRequiredArguments(
      makeHierIndexService(
        ["study_id", "sample_ids"],
        ["study_id", "id"],
        "getSamples"
    ...
    getSamplesByPatient: enforceRequiredArguments(
      makeHierIndexService(
        ["study_id", "patient_ids"],
        ["study_id", "patient_id"],
        "getSamples"
    ...
    getGeneticProfileDataBySample: enforceRequiredArguments(
      makeHierIndexService(
        ["genetic_profile_ids", "genes", "sample_ids"],
        ["genetic_profile_id", "hugo_gene_symbol", "sample_id"],
        "getGeneticProfileData"
      ),
      [
        ["genetic_profile_ids", "genes"],
        ["genetic_profile_ids", "genes", "sample_ids"]
      ]
    ),
    getGeneticProfileDataBySampleList: enforceRequiredArguments(
      makeHierIndexService(
        ["genetic_profile_ids", "genes", "sample_list_id"],
        ["genetic_profile_id", "hugo_gene_symbol", "sample_list_id"],
        "getGeneticProfileData"
      ),
      [
        ["genetic_profile_ids", "genes"],
    ...
    getSampleClinicalAttributes: enforceRequiredArguments(
      function(args) {
        return raw_service.getSampleClinicalAttributes(args);
      },
      [["study_id"], ["study_id", "sample_ids"]]
    ),
    getPatientClinicalAttributes: enforceRequiredArguments(
      function(args) {
        return raw_service.getPatientClinicalAttributes(args);
      },
      [["study_id"], ["study_id", "patient_ids"]]
    ),
    getClinicalAttributesByStudy: enforceRequiredArguments(
      function(args) {
        return raw_service.getClinicalAttributes(args);
      },
      [["study_id"]]
    ),
    getClinicalAttributes: enforceRequiredArguments(
      makeOneIndexService(
        "attr_ids",
        function(d) {
          return d.attr_id;
