...
(function(factory) {
  ...
  $.jstree.core.prototype = {
            ...
            $.each(
              ...
              $.proxy(function(key, val) {
                if (val.children.length === 0) {
                  for (
                    var i = 0, _len = val.parents.length;
                    i < _len - 1;
                    i++
                  ) {
                    this._model.data[val.parents[i]].descendants =
                      this._model.data[val.parents[i]].descendants || [];
      ...
      if ($.isFunction(s)) {
        return s.call(
          ...
          $.proxy(function(d) {
            ...
            ](obj, typeof d === "string" ? $(d) : d, function(status) {
              callback.call(this, status);
            });
    ...
    show_node_decorators: function(obj) {
      ...
      if (node.children.length === 0) {
        if (node.li_attr && node.li_attr.description) {
          var $descriptionIcon = $(
            '<i class="fa fa-lg fa-info-circle jstree-node-decorator" style="cursor:pointer; padding-left:0.4em"></i>'
          );
        ...
        var $linkOutIcon = $(
          '<i class="btn btn-default btn-sm jstree-node-decorator" style="cursor:pointer;  padding: 0px 5px; font-weight: normal;font-style: normal;margin-left: 10px; color:white; background-color:#2986e2">Summary</i>'
        );
        ...
        $linkOutIcon.mousedown(function(e) {
          e.preventDefault();
        });
        ...
        if (this.node_has_descendant_branches(node.id)) {
          ...
          var expandClass = "fa-expand";
          var collapseClass = "fa-compress";
          var $expandCollapseBtn = $(
            '<i class="fa fa-md rotate-45 ' +
              (shouldCollapse ? collapseClass : expandClass) +
              ' jstree-node-decorator" style="display:inline-block; cursor:pointer; padding-left:0.6em"></i>'
          );
          // MEGA HACK by adama@cbio.mskcc.org
          if (obj.children(".jstree-external-node-decorator").length > 0) {
            obj
              .children(".jstree-external-node-decorator")
              .filter(":last")
              .after($expandCollapseBtn);
          } else {
    ...
    is_selected: function(obj) {
      ...
      if (
        this._data.search.str.length > 0 &&
        obj.descendants &&
        obj.descendants.length > 0
      ) {
        for (var i = 0, _len = obj.descendants.length; i < _len; i++) {
          if (
            !m[obj.descendants[i]].state.fixed &&
            !m[obj.descendants[i]].state.selected
          ) {
            return false;
          }
        }
      ...
      $.each(
        ...
        $.proxy(function(key, val) {
          ...
          for (var i = 0, _len = val.descendants.length; i < _len; i++) {
            count += !(
              this._data.search.str.length > 0 &&
              m[val.descendants[i]].state.fixed
            );
          }
            ...
            $.proxy(function(e, data) {
              ...
              this.downward_select = function(parent_obj) {
                ...
                if (is_selected && !parent_obj.state.fixed) {
                  ...
                  if (
                    parent_obj.original &&
                    parent_obj.original.state &&
                    parent_obj.original.state.undetermined
                  ) {
                    parent_obj.original.state.undetermined = false;
                  }
                }
                return parent_obj.state.selected;
              };
              this.downward_select(obj);
              this._data[
                t ? "core" : "checkbox"
              ].selected = $.vakata.array_unique(
                this._data[t ? "core" : "checkbox"].selected
              ...
              /*var all_children_selected = true;
							if(s.indexOf('down') !== -1) {
								search_queue = obj.children_d.slice();
								//this._data[ t ? 'core' : 'checkbox' ].selected = $.vakata.array_unique(this._data[ t ? 'core' : 'checkbox' ].selected.concat(obj.children_d));
								var curr_obj;
								while (search_queue.length > 0) {
									curr_obj = m[search_queue.pop()];
									if (curr_obj.children_d.length === 0) {
										if (!curr_obj.state.fixed) {
											this._data[ t ? 'core' : 'checkbox' ].selected.push(curr_obj.id);
											curr_obj.state[ t ? 'selected' : 'checked' ] = true;
											this.get_node(curr_obj, true).children('.jstree-anchor').addClass(t ? 'jstree-clicked' : 'jstree-checked').parent().attr('aria-selected', true);
											if (curr_obj && curr_obj.original && curr_obj.original.state && curr_obj.original.state.undetermined) {
												curr_obj.original.state.undetermined = false;
											}
										} else {
											all_children_selected = all_children_selected && curr_obj.state.selected;
										}
									}
									search_queue = search_queue.concat(curr_obj.children_d);
								}
								...
								//		this.get_node(tmp, true).children('.jstree-anchor').addClass(t ? 'jstree-clicked' : 'jstree-checked').parent().attr('aria-selected', true);
              ...
              /*// apply down (process .children separately?)
							if(s.indexOf('do wn') !== -1 && dom.length) {
								if (!obj.state.fixed) {
          ...
          if (
            ...
            (!this.settings.dnd.touch ||
              (this.settings.dnd.touch === "selected" &&
                !$(e.currentTarget).hasClass("jstree-clicked")))
          ) {
            return true;
          }
          var obj = this.get_node(e.target),
            mlt =
              this.is_selected(obj) && this.settings.drag_selection
