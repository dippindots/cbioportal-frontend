...
(function() {
  ...
  Chosen = (function(_super) {
    ...
    Chosen.prototype.search_results_mousewheel = function(evt) {
      ...
      delta =
        -((_ref = evt.originalEvent) != null ? _ref.wheelDelta : void 0) ||
      ...
      try {
        for (var i = 0; i < el.length; i++) {
          try {
            ret =
              ret &&
              this.result_compute_activation_down(
                this.container.find(
                  "li[data-parent='" +
    ...
    Chosen.prototype.get_search_results = function(searchText) {
      ...
      regex = new RegExp(
        regexAnchor + searchText.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&"),
        "i"
      );
      zregex = new RegExp(
        ...
        if (!option.empty) {
          if (option.group) {
            ...
            if (regex.test(option.html)) {
              ...
              this.enable_split_word_search &&
              (option.html.indexOf(" ") >= 0 || option.html.indexOf("[") === 0)
            ) {
            ...
            if (found) {
              if (searchText.length && !founddesc) {
                ...
                found_results.push({
                  ...
                  option: option,
                  id: option.dom_id,
                  highlight: [startpos, startpos + searchText.length]
                });
              } else {
                found_results.push({
                  result: result,
    ...
    Chosen.prototype.winnow_results = function() {
      ...
      searchText =
        this.search_field.val() === this.default_text
          ? ""
          : $("<div/>")
              .text($.trim(this.search_field.val()))
              .html();
      ...
      $.each(allResults, function(id, val) {
        ...
        if (
          _this.result_highlight &&
          val.option.dom_id === _this.result_highlight.attr("id")
        ) {
      ...
      for (var i = 0; i < searchTermGroups.positive.length; i++) {
        ...
        for (var j = 0; j < searchWords.length; j++) {
          conjunctions[i].termResults[searchWords[j]] = this.get_search_results(
            searchWords[j]
          );
        }
      }
      for (var i = 0; i < searchTermGroups.negative.length; i++) {
        negativeTermResults[
          searchTermGroups.negative[i]
        ] = this.get_search_results(searchTermGroups.negative[i]);
      }
      ...
      $.each(allResults, function(id, value) {
        ...
        var highlight_map = Array.apply(null, new Array(orig_text.length)).map(
          Number.prototype.valueOf,
          0
        ); // array of 0's, one per index in orig_text
      ...
      for (var i = 0; i < conjunctions.length; i++) {
        $.each(conjunctions[i].foundResults, function(id, _) {
          _this.result_compute_activation(
            allResults[id].result,
            conjunctions[i].activation
          );
        });
      }
      ...
      $.each(allResults, function(id, val) {
        ...
        if (shouldActivate) {
          ...
          if (!val.option.is_group_header && val.result.attr("data-parent")) {
            var curr_result = _this.container.find(
              "li[data-tree-id='" + val.result.attr("data-parent") + "']"
            );
            while (curr_result.length > 0) {
              curr_result.attr(
                "data-desc-studies",
                +curr_result.attr("data-desc-studies") + 1
              );
              curr_result = _this.container.find(
                "li[data-tree-id='" + curr_result.attr("data-parent") + "']"
              );
            }
          }
        }
