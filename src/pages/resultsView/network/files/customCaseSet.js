...
function promptCustomCaseSetBuilder() {
  ...
  $("#case_set_dialog_content").append(
    '<tr><td><img src="files/ajax-loader.gif" alt="loading" /></td></tr>'
  );
  ...
  jQuery.getJSON("ClinicalFreeForm.json", data, function(json) {
    ...
    $("#case_set_dialog_header #number_of_cases").append(
      '<span id="current_number_of_cases">' +
        _customCaseSet.length +
        "</span>" +
        " (out of " +
        json.clinicalCaseSet.length +
        ")"
    );
    ...
    for (var category in categorySet) {
      ...
      if (isEligibleForFiltering(categorySet[category])) {
        ...
        $("#case_set_dialog_content").append(
          '<tr><td align="right">' +
            _humanReadableCategory(category) +
            "</td>" +
            '<td align="left"><select multiple id="select_' +
            category +
            '"></select></td></tr>'
        );
        ...
        for (var i = 0; i < categorySet[category].length; i++) {
          if (
            _customCaseSelection["select_" + category][categorySet[category][i]]
          ) {
            selected = "selected";
          } else {
            selected = "";
...
function _initCustomCaseSelectionMap(categorySet) {
  ...
  for (var category in categorySet) {
    ...
    for (var i = 0; i < categorySet[category].length; i++) {
      _customCaseSelection["select_" + category][
        categorySet[category][i]
      ] = true;
    }
  }
...
function _initCaseSetFilter(categorySet, clinicalCaseSet) {
  ...
  for (var i = 0; i < _freeFormData.length; i++) {
    ...
    if (categoryMap != null && categoryMap[_freeFormData[i].paramValue]) {
...
function refreshCustomCaseSet(checkbox, selector) {
  ...
  if (selectAll) {
    ...
    _customCaseSelection[selector.id][checkbox.val()] = checkbox.prop(
      "checked"
    );
  ...
  $("#case_set_dialog_header #current_number_of_cases").text(
    _customCaseSet.length
  );
}
  ...
  for (var key in caseSetFilter) {
    ...
    if (caseSetFilter[key].length < _clinicalCaseSet.length) {
      ...
      // calling the "expensive" intersect function
        ...
        intersection = _.intersection(intersection, caseSetFilter[key]);
      }
...
function _limitNumberOfCases() {
  ...
  if (_customCaseSet.length > 400) {
    ...
    $("#case_set_dialog_header .custom_case_set_warning").text(
      "Too many cases (max: 400)"
    );
  } else {
