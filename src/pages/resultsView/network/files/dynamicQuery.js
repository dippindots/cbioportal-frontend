...
function loadGeneList(geneSetId) {
  ...
  $.getJSON(
    "portal_meta_data.json?geneset_id=" + geneSetId.replace(/\//g, ""),
    function(json) {
...
function loadMetaData() {
  ...
  function loadContent() {
    ...
    jQuery.getJSON(
      "portal_meta_data.json?partial_studies=true&partial_genesets=true",
      function(json) {
        ...
        $.getJSON(
          "portal_meta_data.json?study_id=" + window.cancer_study_id_selected,
          function(json) {
            console.log(
              "Loading metadata for " + window.cancer_study_id_selected
            );
            // this code should be about the same as in loadStudyMetaData
            window.metaDataJson.cancer_studies[
              window.cancer_study_id_selected
            ] = json;
...
function userClickedMainTab(tabAction) {
  ...
  $("#main_form")
    .get(0)
    .elements["Action"].setAttribute("value", "");
...
function makeDefaultSelections() {
  ...
  $("." + PROFILE_COPY_NUMBER_ALTERATION + ":radio")
    .first()
    .prop("checked", true);
}
...
function reviewCurrentSelections() {
  ...
  if (window.metaDataAdded === true) {
    ...
    toggleThresholdPanel(
      $("." + PROFILE_MRNA_EXPRESSION + "[type=checkbox]"),
      PROFILE_MRNA_EXPRESSION,
      "#z_score_threshold"
    );

    // similarly with RPPA
    toggleThresholdPanel(
      $("." + PROFILE_PROTEIN_EXPRESSION + "[type=checkbox]"),
      PROFILE_PROTEIN_EXPRESSION,
...
function getMapping() {
  ...
  function getMap() {
    ...
    // Get input selection
    var caseIds = $("#custom_case_set_ids")
      .val()
      .trim()
      .replace(/"/g, "")
      .split(/\s+/);
    ...
    if (caseIds[0] !== "") {
      window.cbioportal_client
        .getSamplesByPatient({ study_id: [studyId], patient_ids: caseIds })
        .then(function(sampleMap) {
  ...
  if (
    $("#main_form")
      .find("input[name=patient_case_select]:checked")
      .val() === "patient"
  ) {
    return getMap();
  } else {
...
function chooseAction(evt) {
  ...
  if (!window.changingTabs) {
    ...
    try {
      ...
      if (offset === $("#gene_list").val().length) {
        createAnError(
          "OQL syntax error after selected character; please fix and submit again.",
          $("#gene_list"),
          "oql_error"
        );
        ...
        createAnError(
          "OQL syntax error before selected character; please fix and submit again.",
          $("#gene_list"),
          "oql_error"
        );
  ...
  if (selected_studies.length === 0 && !window.changingTabs) {
    ...
    $("#jstree")
      .jstree(true)
      .select_node(window.jstree_root_id);
    selected_studies = $("#jstree")
      .jstree(true)
      .get_selected_leaves();
  }
  if (selected_studies.length > 1) {
    if (haveExpInQuery) {
      createAnError(
        "Expression filtering in the gene list is not supported when doing cross cancer queries.",
        $("#gene_list")
      );
      return false;
    }
    $("#main_form")
      .find("#select_multiple_studies")
      .val("");
    if ($("#tab_index").val() == "tab_download") {
      $("#main_form")
        .get(0)
        .setAttribute("action", "index.do");
    } else {
      var dataPriority = $("#main_form")
        .find("input[name=data_priority]:checked")
        .val();
      var newSearch =
        $("#main_form").serialize() +
        "&Action=Submit#crosscancer/overview/" +
        dataPriority +
        "/" +
        encodeURIComponent($("#gene_list").val()) +
        "/" +
        encodeURIComponent(selected_studies.join(","));
      evt.preventDefault();
    ...
    $("#main_form")
      .find("#select_single_study")
      .val(selected_studies[0]);
    $("#main_form")
      .get(0)
      .setAttribute("action", "index.do");
    ...
    if (haveExpInQuery) {
      ...
      if (expCheckBox.length > 0 && expCheckBox.prop("checked") == false) {
        createAnError(
          "Expression specified in the list of genes, but not selected in the" +
...
function createAnError(errorText, targetElt, optClassStr) {
  var errorBox = $("<div class='error_box'>").addClass(
    "ui-state-error ui-corner-all exp_error_box"
  );
  if (optClassStr) {
    errorBox.addClass(optClassStr);
  }
  var errorButton = $("<span>").addClass(
    "ui-icon ui-icon-alert exp_error_button"
  );
...
function profileGroupCheckBoxSelected(profileGroup) {
  ...
  if (profileGroup.prop("checked")) {
    var rnaSeqRadios = $(
      "input." + profileClass + "[type=radio][value*='rna_seq']"
    );
    if (rnaSeqRadios.length > 0) {
      rnaSeqRadios.first().prop("checked", true);
    } else {
      $("input." + profileClass + "[type=radio]")
        .first()
        .prop("checked", true);
    }
  }
...
function updateDefaultCaseList() {
  ...
  var mutSelect = $("input.PROFILE_MUTATION_EXTENDED[type=checkbox]").prop(
    "checked"
  );
  var cnaSelect = $("input.PROFILE_COPY_NUMBER_ALTERATION[type=checkbox]").prop(
    "checked"
  );
  var expSelect = $("input.PROFILE_MRNA_EXPRESSION[type=checkbox]").prop(
    "checked"
  );
  var rppaSelect = $("input.PROFILE_PROTEIN_EXPRESSION[type=checkbox]").prop(
    "checked"
  );
  ...
  if (mutSelect && cnaSelect && !expSelect && !rppaSelect) {
    ...
    if (
      $("#select_case_set option[value='" + defaultCaseList + "']").length == 0
    ) {
    ...
    if ($("#" + selectedCancerStudy + "_mrna_median_Zscores").prop("checked")) {
      ...
      $("#" + selectedCancerStudy + "_rna_seq_mrna_median_Zscores").prop(
        "checked"
      )
    ) {
      defaultCaseList = selectedCancerStudy + "_rna_seq_mrna";
    } else if (
      $("#" + selectedCancerStudy + "_rna_seq_v2_mrna_median_Zscores").prop(
        "checked"
      )
    ) {
...
function updateCaseListSmart() {
  ...
  $("#select_case_set_chzn .chzn-drop ul.chzn-results li").each(function(i, e) {
    //make qtip for an element on first mouseenter:
    cbio.util.addTargetedQTip($(e), {
      content:
        "<font size='2'>" +
        $($("#select_case_set option")[i]).attr("title") +
        "</font>",
      style: {
...
function updateCancerStudyInformation() {
  ...
  addGenomicProfiles(
    cancer_study.genomic_profiles,
    "MUTATION_EXTENDED",
    PROFILE_MUTATION_EXTENDED,
    "Mutation"
  );
  addGenomicProfiles(
    cancer_study.genomic_profiles,
    "COPY_NUMBER_ALTERATION",
    PROFILE_COPY_NUMBER_ALTERATION,
    "Copy Number"
  );
  addGenomicProfiles(
    cancer_study.genomic_profiles,
  ...
  jQuery.each(cancer_study.case_sets, function(i, case_set) {
    $("#select_case_set").append(
      "<option class='case_set_option' value='" +
        case_set.id +
        "' title='" +
  ...
  $("#select_case_set").append(
    "<option class='case_set_option' value='-1' " +
  ...
  reviewCurrentSelections(); // cancer study, so disable the button
...
function addMetaDataToPage() {
  ...
  for (var node in oncotree) {
    if (oncotree.hasOwnProperty(node)) {
      oncotree[node].children.sort(function(a, b) {
        try {
          return json.type_of_cancers[a.code].localeCompare(
            json.type_of_cancers[b.code]
          );
        } catch (err) {
  ...
  var truncateStudyName = function(n) {
    ...
    if (n.length < maxLength) {
      ...
      var ellipsis = "... ";
      return (
        n.slice(0, maxLength - suffix.length - ellipsis.length) +
        ellipsis +
        suffix
      );
    }
  ...
  if (window.priority_studies.length > 0) {
    for (var i = 0; i < window.priority_studies.length; i++) {
      ...
      if (
        priority_study_obj.studies.filter(function(s) {
          return json.cancer_studies.hasOwnProperty(s);
        }).length > 0
      ) {
        ...
        jstree_data.push({
          ...
          text: priority_study_obj.category,
          li_attr: { name: priority_study_obj.category }
        });
        $.each(priority_study_obj.studies, function(ind, id) {
          if (json.cancer_studies.hasOwnProperty(id)) {
            ...
            jstree_data.push({
              ...
              li_attr: {
                name: studyName,
                description: metaDataJson.cancer_studies[id].description,
                search_terms: priority_study_obj.category
              }
            });
            ...
            flat_jstree_data.push({
              id: id,
              parent: jstree_root_id,
              text: truncateStudyName(json.cancer_studies[id].name),
              li_attr: {
                name: studyName,
                description: metaDataJson.cancer_studies[id].description,
                search_terms: priority_study_obj.category
  ...
  while (node_queue.length > 0) {
    ...
    if (currNode.desc_studies_count > 0) {
      var name = splitAndCapitalize(
        metaDataJson.type_of_cancers[currNode.code] || currNode.code
      );
      jstree_data.push({
        id: currNode.code,
      ...
      $.each(currNode.studies, function(ind, elt) {
        name = truncateStudyName(
          splitAndCapitalize(metaDataJson.cancer_studies[elt.id].name)
        );
        ...
        jstree_data.push({
          ...
          li_attr: {
            name: name,
            description: metaDataJson.cancer_studies[elt.id].description
          }
        });
  ...
  var matchPhrase = function(phrase, node) {
    ...
    return !!(
      (node.li_attr &&
        node.li_attr.name &&
        node.li_attr.name.toLowerCase().indexOf(phrase) > -1) ||
      (node.li_attr &&
        node.li_attr.description &&
        node.li_attr.description.toLowerCase().indexOf(phrase) > -1) ||
      (node.li_attr &&
        node.li_attr.search_terms &&
        node.li_attr.search_terms.toLowerCase().indexOf(phrase) > -1)
    );
  };
  ...
  var perform_search = function(query) {
    ...
    $.each($("#jstree").jstree(true)._model.data, function(key, node) {
      precomputed_search.results[node.id] = perform_search_single(
        parsed_query,
        node
      );
    });
  ...
  var jstree_search = function(query, node) {
    ...
    if (
      !precomputed_search.results[node.id].result &&
      precomputed_search.results[node.id].forced
    ) {
      return false;
    }
    ...
    $.each(nodes_to_consider, function(ind, elt) {
      ...
      if (
        !precomputed_search.results[elt].result &&
        precomputed_search.results[elt].forced
      ) {
        ret = false;
        return 0;
      }
  ...
  var initialize_jstree = function(data) {
    ...
    $("#jstree").jstree({
      ...
      core: {
        ...
        multiple: window.tab_index !== "tab_download"
      }
    });
    $("#jstree").on("ready.jstree", function() {
      $("#jstree").jstree(true).num_leaves = $("#jstree")
        .jstree(true)
        .get_leaves().length;
      ...
      $("#jstree")
        .jstree(true)
        .get_node(jstree_root_id, true)
        .children(".jstree-anchor")
        .after($jstree_flatten_btn());
      $("#jstree")
        .jstree(true)
        .open_all();
    });
    $("#jstree").on("changed.jstree", function() {
      onJSTreeChange(); /*saveSelectedStudiesLocalStorage();*/
    });
    $("#jstree")
      .jstree(true)
      .hide_icons();
  ...
  initialize_jstree(
    window.tab_index === "tab_download" ? flat_jstree_data : jstree_data
  );
  ...
  var $jstree_flatten_btn = function() {
    ...
    var ret = $(
      '<i class="fa fa-lg fa-code-fork jstree-external-node-decorator" style="display:none; cursor:pointer; padding-left:0.6em"></i>'
    );
  ...
  var do_jstree_search = function() {
    $("#jstree")
      .jstree(true)
      .search($("#jstree_search_input").val());
    $("#jstree")
      .jstree(true)
      .get_node(jstree_root_id, true)
      .children(".jstree-anchor")
  ...
  $("#jstree_search_input").on("input", function() {
    ...
    jstree_search_timeout = setTimeout(function() {
      if ($("#jstree_search_input").val() === "") {
        ...
        $("#jstree").jstree(true)._model.data["tissue"].li_attr.name =
          "All Search Results";
      }
      $("#jstree").fadeTo(100, 0.5, function() {
        ...
        if (
          ...
          $("#jstree").jstree(true)._data.search.res.length === 0
        ) {
  ...
  var onJSTreeChange = function() {
    ...
    var select_multiple_studies = $("#main_form").find(
      "#select_multiple_studies"
    );
    var selected_studies = $("#jstree")
      .jstree(true)
      .get_selected_leaves();
    var selected_ct = selected_studies.length;
    $("#jstree_selected_study_count").html(
      (selected_ct === 0
        ? "No"
        : selected_ct === $("#jstree").jstree(true).num_leaves
        ? "All"
        : selected_ct) +
        " stud" +
        (selected_ct === 1 ? "y" : "ies") +
  ...
  jQuery.each(json.gene_sets, function(key, gene_set) {
    $("#select_gene_set").append(
      "<option value='" + key + "'>" + gene_set.name + "</option>"
    );
  ...
  if (
    !window.cancer_study_list_selected ||
    window.cancer_study_list_selected === ""
  ) {
  ...
  var selected_study_list =
    window.tab_index !== "tab_download"
      ? decodeURIComponent(window.cancer_study_list_selected || "").split(",")
      : [];
  ...
  $("#jstree").on("ready.jstree", function() {
    ...
    $("#select_case_set").chosen({
      width: "420px",
      disable_search_threshold: minSearchableItems,
    ...
    $("#select_single_study").one("doneChanging", function() {
      if (
        window.case_set_id_selected != null &&
        window.case_set_id_selected != ""
      ) {
      ...
      if (
        window.gene_set_id_selected != null &&
        window.gene_set_id_selected != ""
      ) {
...
function addGenomicProfiles(
  genomic_profiles,
  targetAlterationType,
  targetClass,
  targetTitle
) {
  var numProfiles = 0;
  ...
  jQuery.each(genomic_profiles, function(key, genomic_profile) {
    if (genomic_profile.alteration_type == targetAlterationType) {
      if (downloadTab || genomic_profile.show_in_analysis_tab == true) {
        ...
        profileHtml += outputGenomicProfileOption(
          downloadTab,
          optionType,
          targetClass,
          genomic_profile.id,
          genomic_profile.name,
          genomic_profile.description
        );
      }
    }
  }); //  end for each genomic profile loop
  ...
  if (targetClass == PROFILE_MRNA_EXPRESSION && downloadTab == false) {
    ...
    profileHtml +=
      "<div id='z_score_threshold' class='score_threshold'>" +
  ...
  if (targetClass == PROFILE_PROTEIN_EXPRESSION && downloadTab == false) {
    ...
    profileHtml +=
      "<div id='rppa_score_threshold' class='score_threshold'>" +
      "<label>Enter a z-score threshold &#177: " +
      "<input type='text' name='" +
      inputName +
      "' size='6' value='" +
...
function outputGenomicProfileOption(
  downloadTab,
  optionType,
  targetClass,
  id,
