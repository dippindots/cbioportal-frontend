...
var GeneSymbolValidator = (function($) {
  ...
  var validateGenes = function() {
    ...
    try {
      var non_datatypes_lines = oql_parser
        .parse($("#gene_list").val())
        .filter(function(parsed_line) {
      ...
      $("#genestatus").html(
        "<img src='images/ajax-loader2.gif'> <small>Validating gene symbols...</small>"
      );
      var genesStr = non_datatypes_lines
        .map(function(parsed_line) {
      ...
      $.post(
        ...
        function(symbolResults) {
          $("#genestatus").html("");
          var stateList = $("<ul>").addClass(
            "ui-widget icon-collection validation-list"
          );
          var allValid = true;

          // If the number of genes is more than 100, show an error
          if (symbolResults.length > 100) {
            var invalidState = $("<li>").addClass(
              "ui-state-default ui-corner-all"
            );
            var invalidSpan = $("<span>").addClass("ui-icon ui-icon-notice");
            var invalidText = $("<span>").addClass("text");
            invalidText.html("<b>You have entered more than 100 genes.</b>");

            invalidState
              .attr("title", "Please enter fewer genes for better performance.")
              .tipTip();
          ...
          for (var j = 0; j < symbolResults.length; j++) {
            ...
            if (aResult.symbols.length == 1) {
              multiple = false;
              if (
                aResult.symbols[0].toUpperCase() != aResult.name.toUpperCase()
              ) {
                foundSynonym = true;
              } else {
                continue;
            ...
            if (foundSynonym || multiple) allValid = false;

            if (multiple) {
              var state = $("<li>").addClass(
                "ui-state-default ui-corner-all ui-validator"
              );
          ...
          if (allValid) {
            ...
            if (
              ...
              !(
                symbolResults[0].name == "" &&
                symbolResults[0].symbols.length == 0
              )
            ) {
              var validState = $("<li>").addClass(
                "ui-state-default ui-corner-all"
              );
              var validSpan = $("<span>").addClass(
