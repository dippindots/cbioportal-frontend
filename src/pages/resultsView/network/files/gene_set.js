...
GeneSet = function(raw_genes_str) {
  if (typeof raw_genes_str !== "string") {
    throw new Error(
      "Error: GeneSet only takes strings, given type '" +
  ...
  var OncoQuery = function(str, is_datatype) {
    ...
    if (is_datatype) {
      ...
      geneStmt.getQuery = function() {
        return query;
      };
      geneStmt.getGenes = function() {
        return _genes;
      };
    }

    geneStmt.isDatatype = function() {
      return is_datatype;
    };
  ...
  var genes, all_genes, onco_queries;
  ...
  return {
    ...
    toString: function() {
      return $.trim(
        this.getOncoQueries().join(" ") + " " + this.getGenes().join(" ")
      );
    },
