...
function NetworkVis(divId) {
  ...
  this.edgeTypeConstants = {
    CONTROLS_STATE_CHANGE_OF: {
      name: "controls-state-change-of",
      color: "#5F77D4",
      desc: "Controls State Change of"
    },
    CONTROLS_TRANSPORT_OF: {
      name: "controls-transport-of",
      color: "#e96e6e",
      desc: "Controls Transport of"
    },
    CONTROLS_PHOSPHORYLATION_OF: {
      name: "controls-phosphorylation-of",
      color: "#A8AD6F",
      desc: "Controls Phosphorylation of"
    },
    CONTROLS_EXPRESSION_OF: {
      name: "controls-expression-of",
      color: "#62B36D",
      desc: "Controls Expression of"
    },
    CATALYSIS_PRECEDES: {
      name: "catalysis-precedes",
      color: "#4F4F8F",
      desc: "Catalysis Precedes"
    },
    CONSUMPTION_CONTROLED_BY: {
      name: "consumption-controled-by",
      color: "#359C96",
      desc: "Consumption Controled by"
    },
    CONTROLS_PRODUCTION_OF: {
      name: "controls-production-of",
      color: "#397187",
      desc: "Controls Production of"
    },
    CONTROLS_TRANSPORT_OF_CHEMICAL: {
      name: "controls-transport-of-chemical",
      color: "#A968D9",
      desc: "Controls Transport of Chemical"
    },
    USED_TO_PRODUCE: {
      name: "used-to-produce",
      color: "#8F4A86",
      desc: "Used to Produce"
    },
    CHEMICAL_AFFECTS: {
      name: "chemical-affects",
      color: "#8A8262",
      desc: "Chemical Affects"
    },
    IN_COMPLEX_WITH: {
      name: "in-complex-with",
      color: "#825E51",
      desc: "In Complex With"
    },
    INTERACTS_WITH: {
      name: "interacts-with",
      color: "#6AB0CC",
  ...
  this.notHighlightNodeCSS = {
    "border-opacity": 0.3,
    "text-opacity": 0.3,
    "background-opacity": 0.3
  };
  this.notHighlightEdgeCSS = {
    opacity: 0.3,
    "text-opacity": 0.3,
    "background-opacity": 0.3
  };
...
NetworkVis.prototype.initNetworkUI = function(vis) {
  ...
  this.interactionSourceVisibilitySelector = this._createInteractionSourceVisibilityWindow(
    this.divId
  );
  this.interactionTypeVisibilitySelector = this._createInteractionTypeVisibilityWindow(
    this.divId
  );
  this._initDialogs();
...
NetworkVis.prototype._createMergingEdges = function() {
  ...
  for (var i = 0; i < edges.length; i++) {
    if (
      tempEdges[
        "source:" +
          edges[i]._private.data.target +
          ";target:" +
          edges[i]._private.data.source
      ] != undefined
    ) {
      tempEdges[
        "source:" +
...
NetworkVis.prototype.updateSelectedGenes = function(evt) {
  ...
  $(this.geneListAreaSelector + " select option").each(function(index) {
    if ($(this).is(":selected")) {
      var nodeId = $(this).val();
...
NetworkVis.prototype._selectAll_InteractionTypeVisibility = function() {
  for (var key in this.edgeTypeConstants) {
    ...
    if (!this.percentageFilterMap[key]) {
      $(
        this.interactionTypeVisibilitySelector +
          " #" +
          this.edgeTypeConstants[key].name +
          "_check"
      ).attr("checked", true);
...
NetworkVis.prototype._selectAll_InteractionSourceVisibility = function() {
  for (var key in this._edgeSourceVisibility) {
    $(this.interactionSourceVisibilitySelector + " #" + key + "_check").attr(
      "checked",
      true
    );
...
NetworkVis.prototype._unselectAll_InteractionSourceVisibility = function() {
  for (var key in this._edgeSourceVisibility) {
    $(
      this.interactionSourceVisibilitySelector + " #" + key + "_check"
    ).removeAttr("checked");
...
NetworkVis.prototype.saveSettings = function() {
  ...
  for (var i = 0; i < this._layoutOptions.length; i++) {
    ...
    if (this._layoutOptions[i].id == "animate") {
      ...
      this._layoutOptions[i].value = Number(
...
NetworkVis.prototype.updateDetailsTab = function(evt) {
  ...
  var handler = function(queryResult) {
    ...
    if (selected.length != 1 || data.id != selected[0]._private.data.id) {
      return;
    }
  ...
  if (data.type == this.DRUG) {
    ...
    var drugView = new DrugInfoView({
      el: this.detailsTabSelector + " .drug-info-content",
      data: data,
  ...
  // send biogene query for only genes
    ...
    var queryParams = { query: data.label, org: "human", format: "json" };

    $(self.detailsTabSelector + " .biogene-content").append(
      '<img src="files/ajax-loader.gif" alt="loading" />'
    );

    $.post(window.apiHost + "bioGeneQuery.do", queryParams, handler);
  }
};
...
NetworkVis.prototype.addInteractionInfo = function(evt, selector, selectType) {
  ...
  if (evt.cyTarget._private.group == "nodes") return;
  ...
  var showSBGNViewButton = $("<button/>", {
    ...
    click: function() {
      self.popUpSBGNView(sourceLabel, targetLabel, edgeType);
    }
  });
  ...
  if (
    evt.cyTarget._private.data.mergeStatus != undefined &&
    evt.cyTarget._private.data.mergeStatus === "merged"
  ) {
    title += " (Summary Edge)";
...
NetworkVis.prototype._addPubMedIds2 = function(data, summaryEdge) {
  ...
  for (var i = 0; i < ids.length; i++) {
    ...
    xref =
      '<a href="' + link.href + '" target="_blank">' + link.pieces[1] + "</a>";
  ...
  if (summaryEdge) {
    ...
    return this._addDataRow2("edge", "PubMed ID(s)", xrefList);
  }
};
...
NetworkVis.prototype.showEdgeInspector = function(evt) {
  ...
  var title =
    this._vis.$("#" + data.source)._private.data.label +
    " - " +
    this._vis.$("#" + data.target)._private.data.label;

  // clean xref & data rows
  $(
    this.edgeInspectorSelector + " .edge_inspector_content .data .data-row"
  ).remove();
  $(
    this.edgeInspectorSelector + " .edge_inspector_content .xref .xref-row"
  ).remove();
  ...
  // set title
  $(this.edgeInspectorSelector).dialog("option", "title", title);
  ...
  if (
    $(this.edgeInspectorSelector).height() >
    $(this.edgeInspectorSelector).dialog("option", "maxHeight")
  ) {
    $(this.edgeInspectorSelector)
      .dialog("open")
      .height($(this.edgeInspectorSelector).dialog("option", "maxHeight"));
  }
};
...
NetworkVis.prototype.updateGenesTab = function(evt) {
  ...
  if (!this._selectFromTab) {
    ...
    for (var i = 0; i < selected.length; i++) {
      $(
        this.geneListAreaSelector +
          " #" +
          _safeProperty(selected[i]._private.data.id)
...
NetworkVis.prototype.searchGene = function() {
  ...
  for (i = 0; i < genes.length; i++) {
    if (
      genes[i]._private.data.label.toLowerCase().indexOf(query.toLowerCase()) !=
      -1
    ) {
...
NetworkVis.prototype.filterSelectedGenes = function() {
  ...
  this._vis.nodes().forEach(function(ele) {
    if (
      selectionVisibility(ele) === false ||
      ele._private.style.visibility.strValue == "hidden"
    ) {
...
NetworkVis.prototype.updateEdges = function() {
  ...
  for (var key in this.edgeTypeConstants) {
    this._edgeTypeVisibility[
      this.edgeTypeConstants[key].name
    ] = this.visibilityOfType[key];
  }
  ...
  var showAllNodeVisibility = function(element) {
    return (
      self.dropDownVisibility(element) &&
...
NetworkVis.prototype.edgeVisibility = function(element) {
  ...
  if (element._private.data["INTERACTION_DATA_SOURCE"] == null) return;
...
NetworkVis.prototype.dropDownVisibility = function(element) {
  ...
  // (if it is not filtered manually)
    ...
    element._private.data["IN_QUERY"] != null &&
    ...
    if (element._private.data.type == "Drug") {
      if (selectedOption.toString() == "HIDE_DRUGS") {
        ...
        if (element._private.data["CANCER_DRUG"] == "true") visible = true;
        else visible = false;
      } else {
        // check FDA approved
        if (element._private.data["FDA_APPROVAL"] == "true") visible = true;
        else visible = false;
      }
    } else {
      visible = true;
...
NetworkVis.prototype.selectionVisibility = function(element) {
  ...
  // if an edge type is hidden, all edges of that type should be invisible
    ...
    this._edgeTypeVisibility[element._private.data.type] != null &&
  ...
  if (this._removeDisconnected) {
    ...
    this._vis.nodes().forEach(function(ele) {
      ...
      for (var i = 0; i < edges.length; i++) {
        if (
          edges[i].visible() &&
          (ele._private.data.id === edges[i]._private.data.source ||
...
NetworkVis.prototype._highlightNeighbors = function(/*nodes*/) {
  ...
  this._vis
    .nodes(":visible")
    .nodes("[highlighted!='true']")
    .css(this.notHighlightNodeCSS);
  this._vis
    .edges(":visible")
    .edges("[highlighted!='true']")
    .css(this.notHighlightEdgeCSS);
  this._vis
    .nodes(":visible")
    .nodes("[highlighted='true']")
    .removeCss();
 ...
 */
NetworkVis.prototype._addDataRow = function(
  selector,
  type,
  label,
  value /*, section*/
  ...
  var defaultOpts = [
    {
      ...
      label: "Gravity",
      value: 0.4,
      tip: "Gravitonal constant to keep graph components together"
    },
    {
      id: "fit",
      label: "Graph fitting",
      value: true,
      tip: "Whether or not to fit the network into canvas after layout"
    },
    {
      id: "edgeElasticity",
      label: "Edge elasticity",
      value: 0.45,
      tip: "Divisor to computer edge forces"
    },
    {
      id: "animate",
      label: "Animation",
      value: false,
      tip: "Whether or not to display network during layout"
    },
    {
      id: "randomize",
      label: "Randomize",
      value: true,
      tip: "Uncheck for incremental layout"
    },
    { id: "padding", label: "Padding value", value: 10, tip: "Padding on fit" },
    {
      ...
      value: 4500,
      tip: "Node repulsion (non-overlapping) multiplier"
    },
    {
      id: "idealEdgeLength",
      label: "Ideal edge length",
      value: 50,
      tip: "Ideal length of an edge"
    },
    {
      id: "nestingFactor",
    ...
    {
      id: "tile",
      label: "Tile Disconnected",
      value: true,
      tip: "Whether or not to tile disconnected nodes on layout"
    }
  ];
...
NetworkVis.prototype._xrefArray = function() {
  ...
  linkMap["hgnc"] =
    "http://www.genenames.org/cgi-bin/quick_search.pl?.cgifields=type&type=equals&num=50&search=" +
    this.ID_PLACE_HOLDER +
    "&submit=Submit";
  ...
  linkMap["kegg"] =
    "http://www.kegg.jp/dbget-bin/www_bget?dr:" + this.ID_PLACE_HOLDER;
  linkMap["kegg drug"] =
    "http://www.kegg.jp/dbget-bin/www_bget?dr:" + this.ID_PLACE_HOLDER;
  linkMap["chebi"] =
    "http://www.ebi.ac.uk/chebi/searchId.do?chebiId=CHEBI%3A" +
  ...
  linkMap["chemspider"] =
    ...
    this.ID_PLACE_HOLDER +
    ".html";
  linkMap["kegg compund"] =
    "http://www.genome.jp/dbget-bin/www_bget?cpd:" + this.ID_PLACE_HOLDER;
  linkMap["doi"] =
    "http://www.nature.com/nrd/journal/v10/n8/full/nrd3478.html?";
  linkMap["nci_drug"] =
    "http://www.cancer.gov/drugdictionary?CdrID=" + this.ID_PLACE_HOLDER;
  linkMap["national drug code directory"] =
  ...
  linkMap["pdb"] =
    ...
    this.ID_PLACE_HOLDER;
  linkMap["bindingdb"] =
    "http://www.bindingdb.org/data/mols/tenK3/MolStructure_" +
    this.ID_PLACE_HOLDER +
    ".html";
  linkMap["genbank"] =
    "http://www.ncbi.nlm.nih.gov/nucleotide?term=" + this.ID_PLACE_HOLDER;
  linkMap["iuphar"] =
    "http://www.iuphar-db.org/DATABASE/ObjectDisplayForward?objectId=" +
    this.ID_PLACE_HOLDER;
  linkMap["drugs product database (dpd)"] =
    "http://205.193.93.51/dpdonline/searchRequest.do?din=" +
    this.ID_PLACE_HOLDER;
  linkMap["guide to pharmacology"] =
...
NetworkVis.prototype._maxAlterValNonSeed = function(map) {
  ...
  for (var key in map) {
    ...
    if (node != null && node._private.data["IN_QUERY"] == "true") {
      continue;
    }
...
NetworkVis.prototype._initMainMenu = function() {
  ...
  $(this.mainMenuSelector + " #network_menu_topology").addClass(
    this.MENU_CLASS
  );
  ...
  $(this.mainMenuSelector + " #save_as_png").addClass(
    this.MENU_SEPARATOR_CLASS
  );
  $(this.mainMenuSelector + " #save_as_png").addClass(this.LAST_CLASS);

  $(this.mainMenuSelector + " #hide_selected").addClass(this.FIRST_CLASS);
  $(this.mainMenuSelector + " #hide_selected").addClass(
    this.MENU_SEPARATOR_CLASS
  );
  $(this.mainMenuSelector + " #remove_disconnected").addClass(
    this.MENU_SEPARATOR_CLASS
  );
  ...
  $(this.mainMenuSelector + " #show_profile_data").addClass(
    this.MENU_SEPARATOR_CLASS
  );
  $(this.mainMenuSelector + " #highlight_neighbors").addClass(
    this.MENU_SEPARATOR_CLASS
  );
  ...
  $(this.mainMenuSelector + " #perform_layout").addClass(
    this.MENU_SEPARATOR_CLASS
  );
  //$("#layout_properties").addClass(SUB_MENU_CLASS);
  $(this.mainMenuSelector + " #auto_layout").addClass(
    this.MENU_SEPARATOR_CLASS
  );
...
NetworkVis.prototype._updateMenuCheckIcons = function() {
  ...
  if (this._removeDisconnected) {
    ...
    $(this.mainMenuSelector + " #remove_disconnected").removeClass(
      this.CHECKED_CLASS
    );
  }

  if (this._nodeLabelsVisible) {
    $(this.mainMenuSelector + " #show_node_labels").addClass(
      this.CHECKED_CLASS
    );
  } else {
    $(this.mainMenuSelector + " #show_node_labels").removeClass(
      this.CHECKED_CLASS
    );
  }

  if (this._edgeLabelsVisible) {
    $(this.mainMenuSelector + " #show_edge_labels").addClass(
      this.CHECKED_CLASS
    );
  } else {
    $(this.mainMenuSelector + " #show_edge_labels").removeClass(
      this.CHECKED_CLASS
    );
  }

  if (this._panZoomVisible) {
    $(this.mainMenuSelector + " #show_pan_zoom_control").addClass(
      this.CHECKED_CLASS
    );
  } else {
    $(this.mainMenuSelector + " #show_pan_zoom_control").removeClass(
      this.CHECKED_CLASS
    );
  }
  ...
  if (this._profileDataVisible) {
    ...
    $(this.mainMenuSelector + " #show_profile_data").removeClass(
      this.CHECKED_CLASS
    );
  }
};
...
NetworkVis.prototype._initDialogs = function() {
  ...
  $(this.settingsDialogSelector).dialog({
    autoOpen: false,
    resizable: false,
    width: 333
  });
  ...
  $(this.edgeInspectorSelector).dialog({
    autoOpen: false,
    resizable: false,
    width: 366,
    maxHeight: 300
  ...
  // adjust node legend
  $(this.geneLegendSelector).dialog({
    autoOpen: false,
    resizable: false,
    width: 500
  });
  ...
  // adjust drug legend
  $(this.drugLegendSelector).dialog({
    autoOpen: false,
    resizable: false,
    width: 320
  });
  ...
  // adjust edge legend
  $(this.edgeLegendSelector).dialog({
    autoOpen: false,
    resizable: false,
    width: 400
  });
  ...
  $(this.interactionTypeVisibilitySelector).dialog({
    autoOpen: false,
    resizable: false,
    width: 300,
    close: function(event, ui) {
      self._closeInteractionTypePopUp(false);
    }
  });

  //adjust edge source UI visibility dialog
  $(this.interactionSourceVisibilitySelector).dialog({
    autoOpen: false,
    resizable: false,
    width: 300,
    close: function(event, ui) {
      self._closeInteractionSourcePopUp(false);
    }
  });
};
...
NetworkVis.prototype._initSliders = function() {
  ...
  $(this.genesTabSelector + " #affinity_slider_field").keypress(
    keyPressListener
  );
...
NetworkVis.prototype._affinitySliderMove = function(event, ui) {
  ...
  // update current value field
  $(this.genesTabSelector + " #affinity_slider_field").val(
    (sliderVal / 100).toFixed(2)
  );
};
...
NetworkVis.prototype._keyPressListener = function(event) {
  ...
  if (event.keyCode == this.ENTER_KEYCODE) {
    if (event.target.id == "weight_slider_field") {
      ...
      $(this.genesTabSelector + " #affinity_slider_bar").slider(
        "option",
        "value",
...
NetworkVis.prototype._initGenesTab = function() {
  ...
  $(this.genesTabSelector + " #filter_genes").button({
    icons: { primary: "ui-icon-circle-minus" },
    text: false
  });

  $(this.genesTabSelector + " #crop_genes").button({
    icons: { primary: "ui-icon-crop" },
    text: false
  });

  $(this.genesTabSelector + " #unhide_genes").button({
    icons: { primary: "ui-icon-circle-plus" },
    text: false
  });

  $(this.genesTabSelector + " #search_genes").button({
  ...
  $(this.genesTabSelector + " #re-submit_query").button({
...
NetworkVis.prototype._refreshGenesTab = function() {
  ...
  $(this.geneListAreaSelector).append(
    '<select multiple title="Select genes"></select>'
  );
  ...
  for (var i = 0; i < geneList.length; i++) {
    ...
    $(this.geneListAreaSelector + " select").append(
      '<option id="' +
        ...
        classContent +
        'value="' +
        geneList[i]._private.data.id +
        '" ' +
        ">" +
        "<label>" +
        geneList[i]._private.data.label +
        "</label>" +
        "</option>"
    );
...
NetworkVis.prototype._refreshRelationsTabUIVisibility = function() {
  ...
  for (var key in this.visibilityOfType) {
    if (this.visibilityOfType[key] && !this.percentageFilterMap[key]) {
      ...
      this._setComponentVis(
        $(this.relationsTabSelector + " ." + this.edgeTypeConstants[key].name),
        true
      );
...
NetworkVis.prototype._initRelationsTab = function() {
    ...
    $(this.relationsTabSelector + " #edge_type_filter").append(
      '<tr  class="' +
        ...
        <td class="edge-type-checkbox" style="padding-left: 10px;">\
          <label>' +
        this.edgeTypeConstants[key].desc +
        '</label>\
        </td>\
      </tr>\
      <tr class="' +
  ...
  $(this.relationsTabSelector + " #edge_type_filter").append(
    '<tr><td><button id="showInteractionTypeVisibilityUI" style="margin-top: 10px; margin-bottom: 20px;" type="button">Modify...</button></td></tr>'
  );
  ...
  $(this.relationsTabSelector + " #edge_source_filter").append(
    '<tr><td><button id="showInteractionSourceVisibilityUI" style="margin-top: 10px" type="button">Modify...</button></td></tr>'
  );
};
...
NetworkVis.prototype._refreshRelationsTab = function() {
  ...
  for (var key in this.edgeTypeConstants) {
    percent =
      (percentages[this.edgeTypeConstants[key].name] * 100) / edges.length;

    $(
      this.relationsTabSelector +
        " ." +
        this.edgeTypeConstants[key].name +
        " .percent-bar"
    ).css("width", Math.ceil(percent * 0.85) + "%");
    ...
    $(
      this.relationsTabSelector +
        ...
        this.edgeTypeConstants[key].name +
        " .percent-bar"
    ).css("background-color", this.edgeTypeConstants[key].color);
...
NetworkVis.prototype._initControlFunctions = function() {
  ...
  $(this.settingsDialogSelector + " #default_layout_settings").click(
    defaultSettings
  );
  ...
  $("#showInteractionSourceVisibilityUI").click(
    showInteractionSourceVisibility
  );

  $(this.interactionTypeVisibilitySelector + " input").change(function() {
  ...
  $(this.interactionSourceVisibilitySelector + " input").change(function() {
    var isChecked = $(this).is(":checked");
    var key = $(this)
      .attr("id")
      .substr(
  ...
  $(
    this.interactionTypeVisibilitySelector +
      " #close_InteractionTypeVisibilityButton"
  ).click(closeInteractionTypePopUp);
  $(
    this.interactionTypeVisibilitySelector +
      " #selectAll_InteractionTypeVisibilityButton"
  ).click(selectAll_InteractionTypeVisibility);
  $(
    this.interactionTypeVisibilitySelector +
      " #unselectAll_InteractionTypeVisibilityButton"
  ).click(unselectAll_InteractionTypeVisibility);

  $(
    this.interactionSourceVisibilitySelector +
      " #close_InteractionSourceVisibilityButton"
  ).click(closeInteractionSourcePopUp);
  $(
    this.interactionSourceVisibilitySelector +
      " #selectAll_InteractionSourceVisibilityButton"
  ).click(selectAll_InteractionSourceVisibility);
  $(
    this.interactionSourceVisibilitySelector +
      " #unselectAll_InteractionSourceVisibilityButton"
  ).click(unselectAll_InteractionSourceVisibility);

  // add listener for double click action

  this._vis.on("doubleTap", "node", showNodeDetails);

  this._vis.on("doubleTap", "edge", showEdgeDetails);

  // add listener for edge select & deselect actions
  this._vis.on("select", "edge", handleEdgeSelect);

  this._vis.on("deselect", "edge", handleEdgeSelect);

  // add listener for node select & deselect actions
  this._vis.on("select", "node", handleNodeSelect);

  this._vis.on("deselect", "node", handleNodeSelect);
...
NetworkVis.prototype._visibleGenes = function() {
  ...
  for (var i = 0; i < nodes.length; i++) {
    ...
    if (
      this._alreadyFiltered[nodes[i]._private.data.id] == null &&
...
NetworkVis.prototype._createEdgeInspector = function(divId) {
  ...
  var html =
    '<div id="' +
    id +
    '" class="network_edge_inspector hidden-network-ui" title="Edge Inspector">' +
...
NetworkVis.prototype._createGeneLegend = function(divId) {
  ...
  var html =
    '<div id="' +
    id +
    '" class="network_node_legend hidden-network-ui" title="Gene Legend">' +
    '<div id="node_legend_content" class="content ui-widget-content">' +
    '<img src="files/gene_legend.png" alt="gene legend"/>' +
    "</div>" +
    "</div>";
...
NetworkVis.prototype._createDrugLegend = function(divId) {
  ...
  var html =
    '<div id="' +
    id +
    '" class="network_drug_legend hidden-network-ui" title="Drug Legend">' +
    '<div id="drug_legend_content" class="content ui-widget-content">' +
    '<img src="files/drug_legend.png" alt="drug legend"/>' +
    "</div>" +
    "</div>";
...
NetworkVis.prototype._createEdgeLegend = function(divId) {
  ...
  var html =
    '<div id="' +
    id +
    '" class="network_edge_legend hidden-network-ui" title="Interaction Legend">' +
    '<div id="edge_legend_content" class="content ui-widget-content">' +
    '<img src="files/interaction_legend.png" alt="interaction legend"/>' +
    "</div>" +
    "</div>";
...
NetworkVis.prototype._createSettingsDialog = function(divId) {
  ...
  var html =
    '<div id="' +
    id +
    '" style="width: auto;" class="settings_dialog hidden-network-ui" title="Layout Properties">' +
...
NetworkVis.prototype._createInteractionSourceVisibilityWindow = function(
  divId
) {
  ...
  for (var key in this._edgeSourceVisibility) {
    ...
    table +=
      ...
      '<input id="' +
      key +
      '_check" type="checkbox" checked="checked">' +
      '<label style="font-size: 10pt; font-weight: normal;">' +
      key +
      "</label>" +
      "</td>";
  }
  ...
  var html =
    '<div id="' +
    id +
    '" title="Interactions to Show by Source">' +
...
NetworkVis.prototype._createInteractionTypeVisibilityWindow = function(divId) {
  ...
  var html =
    '<div id="' +
    id +
    '" title="Interactions to Show by Type">' +
 ...
 */
NetworkVis.prototype.popUpSBGNView = function(
  sourceNodeID,
  targetNodeID,
  edgeType
) {
  ...
  windowObjectReference = window.open(
    sbgnPageURL + "?" + sourceNodeID + "&" + targetNodeID,
    "SBGN View",
    strWindowFeatures
  );
};
...
function _parseBioGeneXml(xmlDoc) {
  ...
  var returnCode = xmlDoc.getElementsByTagName("return_code")[0].childNodes[0]
    .nodeValue;
  ...
  if (geneAliasesNode.length > 0)
    json.geneAliases = _parseDelimitedInfo(
      geneAliasesNode[0].childNodes[0].nodeValue,
      ":",
      ","
    );
  ...
  if (geneDesignationsNode.length > 0)
    json.geneDesignations = _parseDelimitedInfo(
      geneDesignationsNode[0].childNodes[0].nodeValue,
      ":",
      ","
    );
...
function _reverseTransformValue(value) {
  ...
  var reverse = _solveCubic(0.000230926, -0.0182175, 0.511788, -value);
...
function _solveCubic(a, b, c, d) {
  ...
  var x = _cubeRoot(q + sqrt) + _cubeRoot(q - sqrt) + p;
