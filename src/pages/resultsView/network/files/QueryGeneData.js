...
QueryGeneData = function(data) {
  ...
  var data_values = {
    // unfortunately, this is copied right out of GeneEventImpl.java
    cna: [
      "AMPLIFIED",
      "GAINED",
      "DIPLOID",
  ...
  that.getGeneList = function() {
    return getMapAsList(data.hugo_to_gene_index);
  };
  ...
  that.unaltered_samples = (function() {
    return sample_list.filter(function(sample) {
      return !that.isSampleAltered(sample);
    });
  })();
    ...
    samples_l.forEach(function(i) {
      ...
      for (var g in genes) {
        ...
        if ($.isEmptyObject(range.mutations)) {
          appendToMap(range.mutations, genes[g].mutation);
        }
      }
    });
