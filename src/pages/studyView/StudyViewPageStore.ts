...
export class StudyViewPageStore {
  ...
  constructor() {
    ...
    ); // whenever any data filters change, reset survival analysis settings
    this.reactionDisposers.push(
      reaction(
  ...
  @observable private chartsDimension: {
    [uniqueKey: string]: ChartDimension;
  } = {};
    ...
    if (query.filters) {
      ...
      if (matchedAttr !== undefined) {
        ...
        this.pageStatusMessages["unknownClinicalAttribute"] = {
          message: `The clinical attribute ${
            query.filterAttributeId
  ...
  get containerWidth(): number {
    return (
      this.studyViewPageLayoutProps.cols * STUDY_VIEW_CONFIG.layout.grid.w +
      (this.studyViewPageLayoutProps.cols + 1) *
  ...
  public clinicalDataCountPromises: {
    [id: string]: MobxPromise<ClinicalDataCountWithColor[]>;
  } = {};
  public customChartsPromises: {
    [id: string]: MobxPromise<ClinicalDataCountWithColor[]>;
  } = {};

  @observable.ref private _analysisGroupsClinicalAttribute:
    | ClinicalAttribute
    | undefined;
  @observable.ref private _analysisGroups:
    | ReadonlyArray<AnalysisGroup>
    | undefined;
  ...
  onCheckGene(hugoGeneSymbol: string) {
    ...
    this.queriedGeneSet.set(
      hugoGeneSymbol,
      !this.queriedGeneSet.get(hugoGeneSymbol)
    );
  }

  @computed get selectedGenes(): string[] {
    return this.queriedGeneSet
      .keys()
      .filter(gene => !!this.queriedGeneSet.get(gene));
  }
  ...
  updateSelectedGenes(query: SingleGeneQuery[], genesInQuery: Gene[]) {
    ...
    this.queriedGeneSet = new ObservableMap(
      stringListToSet(genesInQuery.map(gene => gene.hugoGeneSymbol))
    );
  }
      ...
      return {
        groups: [
          ...
          },
          {
            ...
            color: this.chartsAreFiltered
              ? STUDY_VIEW_CONFIG.colors.theme.selectedGroup
              : STUDY_VIEW_CONFIG.colors.theme.unselectedGroup,
        ...
        const data = await defaultClient.fetchClinicalDataUsingPOST({
          ...
          clinicalDataMultiStudyFilter: {
            ...
            identifiers: attr.patientAttribute
              ? this.selectedPatients.map(p => ({
                  entityId: p.patientId,
                  studyId: p.studyId
                }))
              : this.selectedSamples.result!.map(p => ({
                  entityId: p.sampleId,
                  studyId: p.studyId
                }))
          },
          ...
          _.reduce(
            ...
            (map, sample) => {
              map[sample.uniqueSampleKey] =
                patientToAnalysisGroup[sample.uniquePatientKey];
              return map;
            },
            {} as { [sampleKey: string]: string }
          )
  ...
  public getClinicalDataCount(chartMeta: ChartMeta) {
    ...
    if (!this.clinicalDataCountPromises.hasOwnProperty(uniqueKey)) {
      ...
      >({
        ...
        invoke: async () => {
          let dataType = chartMeta.clinicalAttribute!.patientAttribute
            ? "PATIENT"
            : "SAMPLE";
  ...
  @autobind
  @action
  addCustomChart(newChart: NewChart) {
  ...
  updateCustomSelect(newChart: NewChart) {
    const sampleIdentifiers = _.reduce(
      ...
      (acc, next) => {
        acc.push(
          ...next.cases.map((customCase: CustomChartIdentifier) => {
            return {
      ...
      (acc, [chartUniqueKey, visible]) => {
        if (visible && this.chartMetaSet[chartUniqueKey]) {
          ...
          if (dimension !== undefined) {
            ...
            chartMeta.dimension = STUDY_VIEW_CONFIG.layout.dimensions[
              chartMeta.chartType
            ] || { w: 1, h: 1 };
          }
  ...
  updateChartStats() {
    ...
    if (!_.isEmpty(this.mutationProfiles.result)) {
      ...
      this.chartsDimension[UniqueKey.MUTATED_GENES_TABLE] =
    ...
    if (!_.isEmpty(this.cnaProfiles.result)) {
      ...
      this.chartsDimension[UniqueKey.CNA_GENES_TABLE] =
  ...
  initializeChartStatsByClinicalAttributes() {
    ...
    this.clinicalAttributes.result.forEach((obj: ClinicalAttribute) => {
      ...
      if (obj.datatype === "NUMBER") {
        ...
        this.chartsDimension[uniqueKey] =
    ...
    this.chartsDimension[UniqueKey.OVERALL_SURVIVAL] =
    ...
    this.chartsDimension[UniqueKey.DISEASE_FREE_SURVIVAL] =
      STUDY_VIEW_CONFIG.layout.dimensions[ChartTypeEnum.SURVIVAL];
    if (
      dfsStatusFlag &&
      dfsMonthsFlag &&
    ...
    this.chartsDimension[UniqueKey.MUTATION_COUNT_CNA_FRACTION] =
  ...
  changeChartType(attr: ChartMeta, newChartType: ChartType) {
    ...
    if (newChartType === ChartTypeEnum.TABLE) {
      ...
      this.chartsDimension[attr.uniqueKey] =
    ...
    this.currentFocusedChartByUser.dimension = this.chartsDimension[
      attr.uniqueKey
    ];
  }
  ...
  readonly samplesWithNAInSelectedClinicalData = remoteData<Sample[]>({
    ...
    invoke: async () => {
      if (this.chartsAreFiltered) {
        ...
        const uniqueSampleKeysWithoutNA = _.keyBy(
          samplesWithoutNA,
          s => s.uniqueSampleKey
        );
        const samplesWithNA = this.samples.result.filter(
          s => !(s.uniqueSampleKey in uniqueSampleKeysWithoutNA)
        );
  ...
  readonly patientKeysWithNAInSelectedClinicalData = remoteData<string[]>({
    ...
    invoke: () => {
      return Promise.resolve(
        _.uniq(
          this.samplesWithNAInSelectedClinicalData.result!.map(
  ...
  public getCustomChartDownloadData(chartMeta: ChartMeta) {
    return new Promise<string>(resolve => {
      if (
        ...
        if (
          this._customChartsSelectedCases.get(chartMeta.uniqueKey)!.length >
            0 &&
          this._customChartsSelectedCases.get(chartMeta.uniqueKey)![0]
            .patientAttribute
        ) {
          isPatientChart = true;
          header.push("Sample ID");
    ...
    if (this.survivalData.result) {
      ...
      data.push(
        [
          "Study ID",
          "Patient ID",
          ..._.values(uniqueClinicalAttributeIds)
        ].join("\t")
      );
  ...
  public async getMutatedGenesDownloadData() {
    if (this.mutatedGeneData.result) {
      ...
      _.each(this.mutatedGeneData.result, function(
        record: MutationCountByGene
      ) {
        data.push(
          [
            ...
            record.totalCount,
            record.countByEntity,
            getFrequencyStr(record.frequency)
          ].join("\t")
        );
      });
      return data.join("\n");
  ...
  public async getGenesCNADownloadData() {
    if (this.cnaGeneData.result) {
      let data = [
        ["Gene", "Gistic(Q-value)", "Cytoband", "CNA", "#", "Freq"].join("\t")
      ];
    ...
    invoke: async () => {
      return this.survivalPlots.map(obj => {
        ...
        obj.unalteredGroup = getPatientSurvivals(
          this.survivalData.result,
          this.unSelectedPatientKeys.result!,
          obj.associatedAttrs[0],
          obj.associatedAttrs[1],
          s => obj.filter.includes(s)
        );
  ...
  private withMutationDataChartCounts = remoteData<
    ClinicalDataCountWithColor[]
  >({
    invoke: async () => {
