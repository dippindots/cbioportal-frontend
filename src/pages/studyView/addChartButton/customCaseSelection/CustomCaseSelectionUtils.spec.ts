...
describe("CustomCaseSelectionUtils", () => {
  ...
  describe("validateLines", () => {
    ...
    it("In multiple studies, study id needs to be specified", () => {
      ...
      assert.isTrue(
        _.keyBy(result.error, "code")[ErrorCodeEnum.INVALID_CASE_ID] !==
          undefined
      );
    });
    ...
    it("Give error for unknown study ids", () => {
      ...
      assert.isTrue(result.error.length !== 0);
      assert.isTrue(
        _.keyBy(result.error, "code")[ErrorCodeEnum.STUDY_NOT_SELECTED] !==
          undefined
      );
    });
    ...
    it("For duplicate cases, give POTENTIAL_OVERLAP warning when the case is in different group", () => {
      ...
      assert.equal(
        ...
        WarningCodeEnum.POTENTIAL_OVERLAP
      );
    });
    ...
    it("TOO_MANY_INVALID_CASE_ID should be given when there are too many invalid case ids", () => {
      ...
      assert.isTrue(result.error.length !== 0);
      assert.isTrue(
        errors[ErrorCodeEnum.TOO_MANY_INVALID_CASE_ID] !== undefined
      );
