...
export function makeSurvivalChartData(
  ...
  if (naPatientsHiddenInSurvival && patientKeysWithNAInSelectedClinicalData) {
    // filter out NA
    const clinicalNAPatientKeysMap = _.keyBy(
      patientKeysWithNAInSelectedClinicalData
    );
    patientSurvivals = patientSurvivals.filter(s => {
      return (
        patientToAnalysisGroup[s.uniquePatientKey] !== "NA" &&
        !(s.uniquePatientKey in clinicalNAPatientKeysMap)
      );
    });
  }
