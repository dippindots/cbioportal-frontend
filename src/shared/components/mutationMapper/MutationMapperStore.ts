...
export default class MutationMapperStore {
  ...
  readonly swissProtId = remoteData(
    {
      invoke: async () => {
        ...
        const accession: string | string[] = await fetchSwissProtAccession(
  ...
  readonly transcriptsByHugoSymbol = remoteData<
    EnsemblTranscript[] | undefined
  >(
    {
      invoke: async () => {
        if (this.gene) {
          return fetchEnsemblTranscriptsByEnsemblFilter({
  ...
  readonly canonicalTranscript = remoteData<EnsemblTranscript | undefined>(
    {
      ...
      invoke: async () => {
        if (this.gene) {
          return fetchCanonicalTranscriptWithFallback(
                ...
                (gl: GenomicLocation) => {
                  if (
                    ...
                    return this.indexedVariantAnnotations.result[
                      ...
                      .map((tc: TranscriptConsequence) => tc.transcript_id)
  ...
  @computed get filteredHotspotsByProteinPosStart(): {
    [pos: number]: Hotspot[];
  } {
    if (this.indexedHotspotData.result) {
      return groupHotspotsByMutations(
        this.filteredMutationsByPosition,
        this.indexedHotspotData.result,
        defaultHotspotFilter
      );
    } else {
  ...
  @computed get filteredOncoKbDataByProteinPosStart(): {
    [pos: number]: IndicatorQueryResp[];
  } {
    if (this.oncoKbData.result && !(this.oncoKbData.result instanceof Error)) {
      return groupOncoKbIndicatorDataByMutations(
