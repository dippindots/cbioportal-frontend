...
type ImmutablePositionAttr = PositionAttr &
  Immutable.ImmutableObject<PositionAttr>;
...
> {
  ...
  @observable private dataHighlightFilter: (d: Mutation[]) => boolean = () =>
    true;

  @action public setPositionSelected(position: number, newVal: boolean) {
    const toMerge: PositionAttr = {};
    toMerge[position + ""] = newVal;
    this.selectedPositions = this.selectedPositions.merge(
      toMerge
    ) as ImmutablePositionAttr;
  }

  @action public setPositionHighlighted(position: number, newVal: boolean) {
    const toMerge: PositionAttr = {};
    toMerge[position + ""] = newVal;
    this.highlightedPositions = this.highlightedPositions.merge(
      toMerge
    ) as ImmutablePositionAttr;
  }
  ...
  @action public setDataSelectFilter(
    dataSelectorFilter: (d: Mutation[]) => boolean
  ) {
  ...
  @action public setDataHighlightFilter(
  ...
  constructor(data: Mutation[][]) {
    ...
    this.dataSelector = (d: Mutation[]) =>
      !!this.selectedPositions[d[0].proteinPosStart + ""] &&
      this.dataSelectFilter(d);
    this.dataHighlighter = (d: Mutation[]) =>
      !!this.highlightedPositions[d[0].proteinPosStart + ""] &&
      this.dataHighlightFilter(d);
  }
}
