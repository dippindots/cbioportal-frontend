...
var EnhancedFixedDataTable = (function() {
  ...
  var FileGrabber = React.createClass({
    ...
    saveFile: function() {
      ...
      var fileName = this.props.downloadFileName
  ...
  var QtipWrapper = React.createClass({
    ...
    render: function() {
      ...
      if (
        label &&
        shortLabel &&
        label.toString().length > shortLabel.toString().length
      ) {
        qtipFlag = true;
      }
      ...
      return React.createElement(
        "span",
        { className: qtipFlag ? "hasQtip" : "", "data-qtip": label },
        shortLabel
      );
    }
  });
  ...
  var ColumnHider = React.createClass({
    ...
    componentDidMount: function() {
      ...
      // fixed upstream
      $('#hide_column_checklist input[type="checkbox"].checkbox-all').attr(
        "title",
        "Select all"
      );
      $('#hide_column_checklist input[type="text"].search').attr(
        "title",
        "Search"
      );
    },
    ...
    render: function() {
      return React.createElement("div", {
        id: "hide_column_checklist",
        className: "EFDT-top-btn"
      });
    }
  });
  ...
  var PinColumns = React.createClass({
    ...
    componentDidMount: function() {
      ...
      // https://github.com/Nelrohd/bootstrap-dropdown-checkbox/issues/33 is
      // fixed upstream
      $('#pin_column_checklist input[type="checkbox"].checkbox-all').attr(
        "title",
        "Select all"
      );
      $('#pin_column_checklist input[type="text"].search').attr(
        "title",
        "Search"
      );
    },
  ...
  var Filter = React.createClass({
    ...
    componentWillUpdate: function() {
      if (this.props.type === "STRING") {
        if (
          !_.isUndefined(this.props.filter) &&
          this.props.filter.key !== this.state.key &&
    ...
    render: function() {
      if (this.props.type === "NUMBER" || this.props.type === "PERCENTAGE") {
        ...
        return React.createElement(
          "div",
          { className: `${styles.header - filters} pull-right` },
          ...
          <div className={`${styles.headerFilters} pull-right`}>
            <div className={styles.searchWrapper}>
              ...
              <input
                ...
                data-column={this.props.name}
                placeholder={
                  this.props.hasOwnProperty("placeholder")
        ...
        <div className={`${styles.tableControls} clearfix`}>
          ...
          {this.props.filter === "ALL" || this.props.filter === "GLOBAL" ? (
            <Filter
              type="STRING"
              name="all"
              className="EFDT-filter pull-right"
          ...
          {this.props.resultInfo ? (
            <div className={`${styles.resultInfo}`}>
              Showing {this.props.filteredRowsSize} results
            </div>
          ) : null}
        </div>
      );
  ...
  var HeaderWrapper = React.createClass({
    ...
    render: function() {
      ...
      const sortIcon = columnData.sortFlag ? (
  ...
  var CustomizeCell = React.createClass({
    ...
    render: function() {
      ...
      var flag =
        data[rowIndex][field] && filterAll.length > 0
          ? data[rowIndex][field]
              .toLowerCase()
              .indexOf(filterAll.toLowerCase()) >= 0
          : false;
  ...
  var Main = React.createClass({
    ...
    getColumnWidth: function(cols, rows, measureMethod, columnMinWidth) {
      ...
      if (self.props.autoColumnWidth) {
            ...
            if (data) {
              ...
              switch (measureMethod) {
                ...
                default:
                  ...
                  rulerWidth =
                    upperCaseLength * 10 +
                    (dataLength - upperCaseLength) * 8 +
                    15;
                  break;
              }

              columnWidth[attr] =
                columnWidth[attr] < rulerWidth ? rulerWidth : columnWidth[attr];
            }
          });
        });
        ...
        columnWidth = _.object(
          _.map(columnWidth, function(length, attr) {
            return [
              attr,
              length > self.props.columnMaxWidth
        ...
        _.each(row, function(content, attr) {
          ...
          if (_label) {
            ...
            if (_labelWidth > columnWidth[attr]) {
              var end =
                Math.floor((_label.length * columnWidth[attr]) / _labelWidth) -
                3;
        ...
        if (_label) {
          ...
          switch (measureMethod) {
            ...
            default:
              ...
              var dataLength = _label.length;
              _labelWidth =
                upperCaseLength * 10 + (dataLength - upperCaseLength) * 8 + 40;
              break;
          }
          if (_labelWidth > columnWidth[col.name]) {
            var end =
              Math.floor(
                (_label.length * columnWidth[col.name]) / _labelWidth
              ) - 3;
            ...
            if (filters[col].type == "STRING") {
              if (!row[col] && hasGroupHeader) {
                ...
                if (
                  ...
                  row[col]
                    .toLowerCase()
                    .indexOf(filters[col].key.toLowerCase()) < 0
                ) {
                  return false;
                }
                if (
                  row[col] &&
                  row[col].toLowerCase().indexOf(filterAll.toLowerCase()) >= 0
                ) {
                  allFlag = true;
                }
              }
            } else if (
              filters[col].type === "NUMBER" ||
              filters[col].type == "PERCENTAGE"
            ) {
              var cell = _.isUndefined(row[col])
                ? row[col]
                : Number(row[col].toString().replace("%", ""));
              if (!isNaN(cell)) {
                if (hasGroupHeader) {
                  if (
                    filters[col].min !== filters[col]._min &&
                    Number(cell) < filters[col].min
                  ) {
                    return false;
                  }
                  if (
                    filters[col].max !== filters[col]._max &&
                    Number(cell) > filters[col].max
                  ) {
                    return false;
                  }
                }
    ...
    sortRowsBy: function(filteredRows, sortBy, switchDir) {
      ...
      filteredRows.sort(function(a, b) {
        ...
        if (
          typeof aVal != "undefined" &&
          !isNaN(aVal) &&
          typeof bVal != "undefined" &&
          !isNaN(bVal)
        ) {
          if (aVal > bVal) {
            sortVal = 1;
    ...
    onFilterKeywordChange: function(e) {
      ...
      var id = setTimeout(function() {
        var filterAll = self.state.filterAll,
    ...
    getInitialState: function() {
      var cols = [],
        ...
        newCol,
        measureMethod =
          dataLength > 100000 || !this.props.autoColumnWidth
            ? "charNum"
            : "jquery",
      ...
      for (i = 0; i < cols.length; i++) {
        ...
        if (col.type == "NUMBER" || col.type == "PERCENTAGE") {
          ...
          for (var j = 0; j < rows.length; j++) {
            cell = _.isUndefined(rows[j][col.name])
              ? rows[j][col.name]
              : rows[j][col.name].toString().replace("%");
      ...
      var columnWidths = this.getColumnWidth(
        cols,
        rows,
        measureMethod,
        columnMinWidth
      );
      var shortLabels = this.getShortLabels(
        rows,
        cols,
        columnWidths,
        measureMethod
    ...
    render: function() {
      var sortDirArrow =
        this.state.sortDir === this.SortTypes.DESC
          ? "fa fa-sort-desc"
          : "fa fa-sort-asc";
