...
export function makeGeneticTrackWith({
  ...
  return function makeTrack(
    ...
    if (alterationInfo.sequenced !== 0) {
      ...
      infoTooltip = `altered / profiled = ${alterationInfo.altered} / ${
        alterationInfo.sequenced
      }`;
    }
    if (
  ...
  return remoteData<IGeneHeatmapTrackSpec[]>({
    ...
    invoke: async () => {
      ...
      const neededGenes = _.flatten(
        molecularProfileIdToHeatmapTracks.values().map(v => v.genes.keys())
      );
      const genes = await oncoprint.props.store.geneCache.getPromise(
        neededGenes.map(g => ({ hugoGeneSymbol: g })),
        true
      );

      const cacheQueries = _.flatten(
        molecularProfileIdToHeatmapTracks.entries().map(entry =>
