...
export function oncogenicImageClassNames(
  oncogenic: string,
  isVUS: boolean,
...
export function generateTreatments(evidenceTreatments: any) {
  ...
  _.each(evidenceTreatments, function(content, type) {
    _.each(content, function(item, index) {
      ...
      if (
        !treatments[_level][_alterations][_treatment].hasOwnProperty(_tumorType)
      ) {
    ...
    _.each(_.keys(treatments[level]).sort(), function(_alteration) {
      _.each(_.keys(treatments[level][_alteration]).sort(), function(
        ...
        _.each(
          ...
          function(_tumorType) {
            var content =
              treatments[level][_alteration][_treatment][_tumorType];
            result.push({
              level: content.level,
