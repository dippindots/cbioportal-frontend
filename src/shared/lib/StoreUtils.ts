...
export async function fetchUniprotId(swissProtAccession: string) {
  const uniprotData: Response = await request.get(
    getUniprotIdUrl(swissProtAccession)
  );
 ...
 * length.
 */
export async function fetchCanonicalTranscriptWithFallback(
  hugoSymbol: string,
  isoformOverrideSource: string,
  allTranscripts: EnsemblTranscript[] | undefined,
  client: GenomeNexusAPI = genomeNexusClient
) {
  return fetchCanonicalTranscript(
    hugoSymbol,
...
export async function fetchCanonicalTranscript(
  hugoSymbol: string,
  isoformOverrideSource: string,
  client: GenomeNexusAPI = genomeNexusClient
) {
