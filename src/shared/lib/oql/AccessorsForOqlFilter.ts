...
export default class AccessorsForOqlFilter {
  ...
  public mut_type(d: Mutation) {
    if (
      this.molecularAlterationType(d.molecularProfileId) === "MUTATION_EXTENDED"
    ) {
  ...
  public exp(d: NumericGeneMolecularData) {
    if (
      this.molecularAlterationType(d.molecularProfileId) === "MRNA_EXPRESSION"
    ) {
      return d.value;
    } else {
      return null;
  ...
  public prot(d: NumericGeneMolecularData) {
    if (
      this.molecularAlterationType(d.molecularProfileId) === "PROTEIN_LEVEL"
    ) {
      return d.value;
    } else {
      return null;
