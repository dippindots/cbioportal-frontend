...
describe("ExtendedRouterStore", () => {
  ...
  beforeEach(() => {
    ...
    saveRemoteSessionStub = sinon
      .stub(routingStore, "saveRemoteSession")
      .callsFake(function() {
  ...
  it("Updating route results in changed location/query when we are under url length limit, does not use session when threshold not met", () => {
    ...
    assert.equal(
      ...
      "/results",
      "sets path appropriately"
    );
    assert.deepEqual(
      routingStore.location.query,
      { param2: "altered", param3: "new" },
      "removes param1"
    );
  });
  ...
  it("Updating route results in new session with id in url and data in _session when session is enabled", done => {
    ...
    assert.isUndefined(
      ...
      "we start with undefined _session"
    );
    ...
    assert.deepEqual(
      routingStore.location.query,
      { session_id: "pending" },
      "sets session id in url to pending"
    );

    assert.isTrue(
      saveRemoteSessionStub.calledOnce,
      "called saveRemote session"
    );
    ...
    setTimeout(() => {
      assert.equal(
        ...
        "somekey",
        "when session service response it updates url with id"
      );
  ...
  it("Restores non-session behavior", () => {
    ...
    assert.deepEqual(
      mobx.toJS(routingStore.location.query),
      { param2: "altered", param3: "new" },
      "using url again"
    );
  ...
  it("Produces new session when query changes", async () => {
    ...
    assert.equal(
      routingStore.location.query.session_id,
      "somekey",
      "we have non-pending session_id"
    );
  ...
  it("#sessionEnabledForPath handles deep routes", () => {
    ...
    assert.isTrue(
      routingStore.sessionEnabledForPath("/results/"),
      "trailing slash"
    );
    assert.isTrue(
      routingStore.sessionEnabledForPath("/results/oncoprint"),
      "deep"
    );
    assert.isFalse(
      routingStore.sessionEnabledForPath("/notEnabled"),
      "no match"
    );
  });
  ...
  it("#query getter gets query from _session or url depending on session status", () => {
    ...
    assert.deepEqual(mobx.toJS(routingStore.query), {
      param1: 1,
      param2: 2,
      param3: 3
    });
    ...
    assert.deepEqual(
      mobx.toJS(routingStore.query),
      { param2: "altered", param3: "new" },
      "can still get it even though not in location anymore"
    );
  });
  ...
  it("#query getter gets query from _session or url depending on session status", () => {
    ...
    assert.isFalse(
      routingStore.needsRemoteSessionLookup,
    ...
    assert.isFalse(
      routingStore.needsRemoteSessionLookup,
      "session DOES match existing session_id"
    );
    ...
    assert.isTrue(
      routingStore.needsRemoteSessionLookup,
      "session DOES NOT match existing session_id"
    );
  });
});
